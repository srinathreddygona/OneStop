...
function AdvertisementUpload() {
  ...
  useEffect(() => {
    // Extract initial brands from addProductFormElements
    const initialBrandOptions = addProductFormElements.find(
      (element) => element.name === "brand"
    )?.options || [];

    // Get top 2-3 brands from each category
    const additionalBrands = categories.flatMap((category) =>
      getBrandsByCategory(category.id)
        .filter((brand) => !initialBrandOptions.some((b) => b.id === brand.id)) // Avoid duplicates
        .slice(0, 3) // Take top 3 brands
    );

    // Combine initial brands with additional brands, removing duplicates
    const allBrands = [
      ...initialBrandOptions,
      ...additionalBrands,
    ].reduce((unique, brand) => {
